...
# זיהוי שאלות מידעיות - מילות שאלה, סימן שאלה, הקשר של זכויות/מידע
# "מה הזכויות שלי במקרה של אלימות?" → AI, לא תגובת חירום
INFORMATIONAL_PATTERN = re.compile(
    r'(?:'
    r'^(?:מה|איך|האם|למה|מתי|כמה|אילו|מהם|מהן|מהי|מהו|מי)\b|'  # מילת שאלה בתחילת משפט
    r'\?|'                                                          # סימן שאלה
    r'מידע (?:על|לגבי|בנושא)|'                                     # "מידע על..."
    r'(?:זכויות|חוק|הגנה|סיוע|עזרה|טיפול|משאבים).*(?:במקרה|בנושא|לגבי|של)|'  # הקשר זכויות+נושא
    r'(?:מה עושים|איך מתמודדים|לאן פונים)\b'                       # "מה עושים ב..."
    r')',
    re.IGNORECASE
)
...
def detect_crisis_type(text):
    """בדיקה אם ההודעה מכילה סימני משבר או חירום.
    - משבר נפשי (התאבדות/פגיעה עצמית): תמיד מפעיל - בטיחות קודמת
    - חירום רפואי: מפעיל רק אם ההודעה לא נראית כשאלת מידע
    מחזיר 'crisis', 'emergency', או None"""
    # משבר נפשי - תמיד מפעיל, העלות של false positive נמוכה
    if CRISIS_PATTERN.search(text):
        return 'crisis'
    # חירום - רק אם לא שאלת מידע (ה-AI יודע להפנות חירום אמיתי בעצמו)
    if EMERGENCY_PATTERN.search(text) and not INFORMATIONAL_PATTERN.search(text):
        return 'emergency'
    return None
